...
const roadmap: StudyRoadmapIdentity = {
  ...
  itemIds: ['chk_attempt-read', 'chk_attempt-drill', 'chk_attempt-hash'],
  roadmapId: 'roadmap-1',
  ...
  it('reads the published study surface and observes progress advancement', async () => {
    ...
    const agent: AgentSession = {
      ...
      callTool: async <TOutput>(name: string, arguments_: Record<string, unknown>): Promise<TOutput> => {
        ...
        const arraysId = 'sub_attempt-arrays'
        const hashingId = 'sub_attempt-hashing'
        const content = (() => {
          ...
          if (name === 'progress_get') {
            return { roadmap_id: roadmap.roadmapId, total_items: 3, checked_items: updated ? 2 : 0, percent: updated ? 67 : 0, deadline: null, sections: [], checked_ids: updated ? ['chk_attempt-drill', 'chk_attempt-read'] : [] }
          }
          if (name === 'roadmap_get_next') {
            return updated
              ? { complete: false, items: [{ item_id: 'chk_attempt-hash', path_position: 2, resources: [], subsection_id: hashingId, text: 'chk_attempt-hash', why_now: 'Prerequisites are complete' }], remaining_in_path: 1 }
              : { complete: false, items: [{ item_id: 'chk_attempt-read', path_position: 0, resources: [], subsection_id: arraysId, text: 'chk_attempt-read', why_now: 'First item' }, { item_id: 'chk_attempt-drill', path_position: 0, resources: [], subsection_id: arraysId, text: 'chk_attempt-drill', why_now: 'First item' }], remaining_in_path: 2 }
          }
          if (name === 'progress_update') {
            updated = true
            return {
              progress: { roadmap_id: roadmap.roadmapId, total_items: 3, checked_items: 2, percent: 67, deadline: null, sections: [], checked_ids: ['chk_attempt-drill', 'chk_attempt-read'] },
              next: { complete: false, items: [{ item_id: 'chk_attempt-hash', path_position: 2, resources: [], subsection_id: hashingId, text: 'chk_attempt-hash', why_now: 'Prerequisites are complete' }], remaining_in_path: 1 },
            }
          }
    ...
    expect(result.finalProgress.checked_ids).toEqual(['chk_attempt-drill', 'chk_attempt-read'])
    expect(result.initialNext.items.map((item) => item.item_id)).toEqual(['chk_attempt-read', 'chk_attempt-drill'])
    expect(result.finalNext.items.map((item) => item.item_id)).toEqual(['chk_attempt-hash'])
    expect(calls).toEqual([
      ...
      { name: 'roadmap_get_overview', arguments_: { roadmap_id: roadmap.roadmapId } },
      {
        ...
        arguments_: { roadmap_id: roadmap.roadmapId, subsection_id: 'sub_attempt-hashing', format: 'detailed' },
      },
      {
      ...
      { name: 'roadmap_get_next', arguments_: { roadmap_id: roadmap.roadmapId, format: 'detailed' } },
      {
        ...
        arguments_: { roadmap_id: roadmap.roadmapId, item_ids: ['chk_attempt-read', 'chk_attempt-drill'], state: 'complete' },
      },
      { name: 'progress_get', arguments_: { roadmap_id: roadmap.roadmapId, detailed: true } },
      { name: 'roadmap_get_next', arguments_: { roadmap_id: roadmap.roadmapId, format: 'detailed' } },
    ])
