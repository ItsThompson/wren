...
- ORM base and naming convention: `backend/src/wren/core/orm.py`
- Domain models: `backend/src/wren/roadmaps/models.py`,
  `backend/src/wren/progress/models.py`, `backend/src/wren/accounts/models.py`,
  `backend/src/wren/oauth/models.py`
- Migrations: `backend/alembic/versions/*`, `backend/alembic/env.py`
...
one async connection pool (`backend/src/wren/core/db.py`). The MCP server holds no
store of its own.
...
Table `roadmaps`, one row per roadmap (`roadmaps/models.py`).
...
Table `progress`, one row per `(user_id, roadmap_id)` (`progress/models.py`).
...
Six tables back the OAuth 2.1 AS (`oauth/models.py`).
...
- Every domain's ORM models inherit from one `Base` (`core/orm.py`), so
